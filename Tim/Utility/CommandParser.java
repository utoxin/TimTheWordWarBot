...
public class CommandParser {
	public static <T extends GenericChannelEvent & GenericUserEvent & GenericMessageEvent> CommandData parseCommand(T event) {
		...
		commandData.issuer = event.getUser()
								  .getNick();
		event.getUser();
		...
		if (space > 0) {
			commandData.command = message.substring(1, space)
										 .toLowerCase();
			commandData.args = message.substring(space + 1)
									  .split(" ", 0);
			commandData.argString = message.substring(space + 1);
		} else {
			commandData.command = message.substring(1)
										 .toLowerCase();
