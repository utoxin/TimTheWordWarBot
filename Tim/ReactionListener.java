...
class ReactionListener extends ListenerAdapter {
	...
	public void onMessage(MessageEvent event) {
		...
		if (event.getUser() != null && !Tim.db.ignore_list.contains(event.getUser().getNick().toLowerCase()) &&
			...
			if (message.charAt(0) != '$' && message.charAt(0) != '!') {
				...
				if (message.toLowerCase().contains("how many lights") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.lights_odds) {
						event.getChannel().send().message("There are FOUR LIGHTS!");
						cdata.lights_odds--;
					}
				} else if (message.toLowerCase().contains("what does the fox say") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.fox_odds) {
						event.respond("Foxes don't talk. Sheesh.");
						cdata.fox_odds--;
					}
				} else if (message.toLowerCase().contains("groot") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.groot_odds) {
						event.respond("I am groot!");
						cdata.groot_odds--;
					}
				} else if (message.toLowerCase().contains("when will then be now") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.soon_odds) {
						event.respond("Soon.");
						cdata.soon_odds--;
					}
				} else if (message.toLowerCase().contains("raptor") && cdata.chatter_enabled.get("velociraptor") && InteractionControls.interactWithUser(event.getUser().getNick(), "velociraptor")) {
					Tim.raptors.sighting(event);
					Tim.markovProcessor.storeLine("say", message);
				} else if (message.toLowerCase().contains("cheeseburger") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.cheeseburger_odds) {
						event.respond("I can has cheezburger?");
						cdata.cheeseburger_odds--;
					}
				} else if (message.toLowerCase().startsWith("test") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.test_odds) {
						event.respond("After due consideration, your test earned a: " + pickGrade());
						cdata.test_odds--;
					}
				} else if ((message.contains(":(") || message.contains("):")) && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(cdata.lastSpeaker.getNick(), "hugs")) {
					if (Tim.rand.nextInt(100) < cdata.hug_odds) {
						event.getChannel().send().action("gives " + cdata.lastSpeaker.getNick() + " a hug.");
						cdata.hug_odds--;
					}
				} else if (message.contains(":'(") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.tissue_odds) {
						event.getChannel().send().action("passes " + event.getUser().getNick() + " a tissue.");
						cdata.tissue_odds--;
					}
				} else if (Pattern.matches("(?i).*how do (i|you) (change|set) ?(my|your)? (nick|name).*", message) && cdata.chatter_enabled.get("helpful_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "helpful_reactions")) {
					event.respond("To change your name type the following, putting the name you want instead of NewNameHere: /nick NewNameHere");
				} else if (Pattern.matches("(?i).*are you (thinking|pondering) what i.*m (thinking|pondering).*", message) && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
				...
				} else if (Pattern.matches("(?i).*what.*is.*the.*answer.*", message) && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
					if (Tim.rand.nextInt(100) < cdata.answer_odds) {
						event.respond("The answer is 42. Everyone knows that.");
						cdata.answer_odds--;
					}
				} else if (Pattern.matches("(?i)" + Tim.bot.getNick() + ".*[?]", message) && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
	...
	public void onAction(ActionEvent event) {
		...
		if (!Tim.db.ignore_list.contains(event.getUser().getNick().toLowerCase()) &&
			...
			} else if ((message.contains(":(") || message.contains("):")) && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "hugs")) {
				if (Tim.rand.nextInt(100) < cdata.hug_odds) {
					event.getChannel().send().action("gives " + event.getUser().getNick() + " a hug.");
					cdata.hug_odds--;
				}
			} else if (message.toLowerCase().startsWith("tests") && cdata.chatter_enabled.get("silly_reactions") && InteractionControls.interactWithUser(event.getUser().getNick(), "silly_reactions")) {
				if (Tim.rand.nextInt(100) < cdata.test_odds) {
