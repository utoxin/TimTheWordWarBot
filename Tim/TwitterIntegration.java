...
public class TwitterIntegration extends StatusAdapter {
	...
	private StatusListener publicListener = new StatusListener() {
		...
		public void onStatus(Status status) {
			...
			String message = colorString + "@" + status.getUser()
													   .getScreenName() + ": " + Colors.NORMAL + status.getText()
																									   .replaceAll("\\n", " ");

			HashSet<String> channels = accountsToChannels.get(status.getUser()
																	.getId());
			...
			for (String channelName : channels) {
				...
				if (channel.tweetBucket >= 1) {
					Tim.channelStorage.channelList.get(channel.channel)
												  .send()
												  .message(message);
			...
			if (!status.getUser()
					   ...
					   .equals("BotTimmy")) {
				try {
					checkFriendship = twitter.showFriendship(BotTimmy.getId(), status.getUser()
																					 .getId());
					if (status.getText()
							  .toLowerCase()
							  .contains("#nanowrimo") && Tim.rand.nextInt(100) < 3 && checkFriendship.isTargetFollowingSource() && !status.isRetweet()) {
						String message2;
		...
		if (needFilterUpdate) {
			if (publicStream != null) {
				if (reconnectPending) {
					...
					if (nextConnect > 0) {
						Logger.getLogger(TwitterIntegration.class.getName())
							  .log(Level.INFO, "Sleeping for {0} milliseconds before twitter reconnect.", nextConnect);
						try {
			...
			long[] finalUserIds = ArrayUtils.toPrimitive(accountsToChannels.keySet()
																		   .toArray(new Long[0]));
