...
class ChainStory implements CommandHandler {
	...
	public boolean handleCommand(CommandData commandData) {
		...
		switch (commandData.command) {
			...
			case "chainnew":
				if (cdata.commands_enabled.get("chainstory")) {
					if (!isNovember) {
						commandData.getMessageEvent().respond("Sorry, that command won't work outside November!");
						return true;
					}
					addNew(commandData);
				} else {
	...
	private void addNew(CommandData commandData) {
		if (commandData.getUserEvent().getUser() == null) {
			return;
		}
		...
		if ("".equals(message)) {
			commandData.getChannelEvent().getChannel().send().action("blinks, and looks confused. \"But there's nothing there. That won't help my wordcount!\"");
		} else {
			try {
				con = Tim.db.pool.getConnection(timeout);

				storeLine(message, commandData.getMessageEvent().getUser().getNick());
				commandData.getChannelEvent().getChannel().send().action("quickly copies down what " + commandData.getUserEvent().getUser().getNick() + " said. \"Thanks!\"");

				PreparedStatement s = con.prepareStatement("SELECT SUM( LENGTH( STRING ) - LENGTH( REPLACE( STRING ,  ' ',  '' ) ) +1 ) AS word_count FROM story");
				s.executeQuery();
				ResultSet rs = s.getResultSet();
				while (rs.next()) {
					commandData.getMessageEvent().respond("My novel is now " + formatter.format(Integer.parseInt(rs.getString("word_count"))) + " words long!");
				}
