...
public class Tim extends PircBot
{
	...
	/*
	 * Defining these at the top so I can conveniently change them in one place
	 * between checkins
	 */
	...
	/*
	 * private static String dbserver = "192.168.2.51"; private static String
	 * dbdbase = ""; private static String dbuser = ""; private static String
	 * dbpass = "";
	 */
	...
	private static String[] fridgeColours = { "avocado green", "powder blue",
			"pale yellow", "sienna", "crimson", "lime green", "fushia",
			"orangeish yellow", "neon pink", "topaz", "taupe", "silver",
			"anguish", "teal", "aqua", "purple", "beige", "burgundy",
			"scarlet", "navy", "turquoise", "cerulean", "olive", "black",
			"chocolate", "beige", "invisible", "witchling", "maroon",
			"#BBC401", "oxide of chromium", "aubergine", "harvest gold",
			"sparkly vampire", "denim", "dirty yellow", "desert", "dessert",
			"dirt", "#8F223B", "cinnamon", "chrome", "leopard", "forest pink",
			"black", "blacker", "blackest", "poison", "Japan", };
	private static String[] eightballResponses = {
			"Direction unclear",
			"Yes",
			"No",
			"Maybe, if you wiggle it a bit",
			"It depends",
			"Sometimes",
			"Ask later",
			"Absolutely",
			...
			"Only if you hop on one foot and sing a song",
			"Have a drink and ask again",
			"Get me a drink and ask again",
			"Indubidably",
			"What, you never just want to say hi?",
			"Never in your wildest dreams",
			"Only in your wildest dreams",
			"HAHAHAHAHAHAHAHAhahahaaa...  haahaaa...  phew...  Oh yeah, totally do it.",
			"You're joking!",
			"What even is that?",
			"Penguins",
			"Ninjas",
			...
			"Very doubtful",
			"Of course! Wait... what did I just agree to?",
			"Ye... er, I mean no",
			"Absolutely! Not.",
			...
			"Have you tried sidling up to your problem?",
			"I didn't understand the question.", "YA RLY", };
	private static String[] greetings = {
			"Welcome to the Congregation of Chris T. Baty!",
			"Have you backed up your novel today?",
			"Have you thanked an ML or Staff member recently?",
			"Have you donated yet this year?",
	...
	private static String[] commandments = {
			...
			"9. Those that sacrifice their money shall be blessed with a halo of gold, which shall be a sign unto others.",
			"10. <<WRITE THIS LATER>>",
	...
	private static String[] flavours = { "peach fudge", "juniper", "vanilla",
			...
			"pine needles", "mashed porkchops", "torment", "lightbulb",
	...
	public Tim()
	{
		...
		if (nicks instanceof Collection)
		{
			...
			this.password = (String) Tim.config
					.getProperty("nicks.nick(0).pass");
		}
		else
		{
		...
		this.adult_channels = (Collection) Tim.config
				.getProperty("adultchannels.channel");
		...
		this.timer_lock = new Semaphore(1, true);

		this.rand = new Random();
		this.shutdown = false;
		// this.db = new MySQL(Tim.dbserver, Tim.dbserver, Tim.dbuser,
		// Tim.dbpass);
	}
			...
			String message)
	{
		...
		if (prefix.equals("!"))
		{
			...
			else if (command.equals("credits"))
			{
				this.sendMessage(
						channel,
						"I was created by MysteriousAges and Utoxin during NaNoWriMo 2010. My code is based on the PircBot Java library. Sourcecode is available here: https://github.com/utoxin/TimTheWordWarBot");
			}
			else if (command.equals("anything") || command.equals("jack")
					|| command.equals("squat"))
			{
			...
			else if (command.equals("reset"))
			{
						...
						|| this.admin_list.contains(channel))
				{
					...
					this.sendDelayedMessage(channel, "Can you hear me now?",
							2000);
				}
				else
				{
			...
			else if (command.equals("setmode"))
			{
				...
				{
					if (u[i].getNick().equals(sender))
					{
						if (u[i].getPrefix().equals("@") || u[i].getPrefix().equals("~"))
						{
							isOp = true;
						}
					}
				}
				if (isOp || this.admin_list.contains(sender))
				{
					if (args != null && args[0].equalsIgnoreCase("adult"))
					{
						this.adult_channels.add(channel);
						this.sendMessage(channel, "This channel has been adultified.");
					}
					else
					{
						this.adult_channels.remove(channel);
						this.sendMessage(channel, "This channel is no longer adultified.");
					}
				}
				else
				{
					this.sendMessage(channel, sender + ", you don't have authorization to do that.");
				}
			}
			else
			{
	...
	private void lick(String channel, String sender, String[] args)
	{
			...
			if (args != null && args.length >= 1)
			{
				...
				else if (args[0].equalsIgnoreCase(this.getNick()))
				{
					this.sendAction(channel, "licks " + args[0]
							+ ". Tastes like tastes like tastes like meta.");
				}
				...
				else
				{
					this.sendAction(
							...
							"licks "
									+ argStr
									+ ". Tastes like "
									+ Tim.flavours[this.rand
											.nextInt(Tim.flavours.length - 1)]);
				}
			}
			else
			{
		...
		{
			this.sendMessage(channel, "This command has been disabled here.");
		}
	}
	...
	private void commandment(String channel, String sender, String[] args)
	{
		...
		if (args != null && args.length == 1 && Double.parseDouble(args[0]) > 0
				&& Double.parseDouble(args[0]) <= Tim.commandments.length)
		{
			...
			Boolean righto)
	{
		...
		String colour = Tim.fridgeColours[this.rand
				.nextInt(Tim.fridgeColours.length - 1)];
			...
			Boolean righto)
	{
		...
		String act = "";
		String colour = Tim.fridgeColours[this.rand
				.nextInt(Tim.fridgeColours.length)];
	...
	private void fridge(String channel, String sender, String[] args)
	{
		if (args != null)
		{
			...
			this.sendAction(channel, "gets " + sender + " a fresh cup of "
					+ drink);
		}
		else
		{
			this.sendAction(channel, "gets " + sender
	...
	private void fridgeFor(String channel, String sender, String[] args)
	{
		if (args != null && args.length > 0)
		{
			if (args.length > 1)
			{
				...
				this.sendAction(channel, "gets " + args[0] + " a fresh cup of"
						+ drink);
			}
			else
			{
				this.sendAction(channel, "gets " + args[0]
		...
		else
		{
			this.sendMessage(channel, sender
					+ ": I need someone to give something to.");
		}
	}
	...
	private void warStartCount(WordWar war)
	{
		this.sendMessage(war.getChannel(), war.getName()
				+ ": Starting in "
				+ (war.time_to_start == 60 ? "one minute" : war.time_to_start
	...
	private void connectToServer()
	{
		...
		if (channels instanceof Collection)
		{
			...
			{
				this.joinChannel((String) Tim.config
						.getProperty("channels.channel(" + i + ")"));
			}
		}
		else if (channels instanceof String)
		{
			this.joinChannel((String) Tim.config
					.getProperty("channels.channel"));
		}
	}
	...
	public static void main(String[] args)
	{
		...
		/*
		 * try { System.setOut(new java.io.PrintStream("Timmy.log.txt")); }
		 * catch (IOException e) { }
		 */
