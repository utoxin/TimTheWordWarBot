...
public class Tim extends PircBot
{
	...
	private static String[] eightballResponses = {
			...
			"Absolutely",
			"Porcupine",
			"Captian Picard is not aboard the Enterprise",
			"Probably",
			"I think so",
			"It may be difficult",
			"No problem",
			...
			String message)
	{
		...
		if (prefix.equals("!"))
		{
			 ...
			 * ticker.cancel(); ticker = null; ticker = new Timer(true); ticker
			 * .scheduleAtFixedRate (warticker, 0, 1000); } }
			 */
			else
			{
		...
		if (args != null && args.length > 0)
		{
			...
			if (this.wars.containsKey(name.toLowerCase()))
			{
				if (sender.equalsIgnoreCase(this.wars.get(name.toLowerCase())
						.getStarter()) || this.admin_list.contains(sender))
				{
