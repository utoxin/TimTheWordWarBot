...
public class Challenge implements ICommandHandler {
		...
		switch (commandData.command) {
			case "challenge":
				if (cdata.commands_enabled.get("challenge")) {
					issueChallenge(commandData.getChannelEvent()
			...
			case "challengefor":
				...
				if (cdata.commands_enabled.get("challenge")) {
					if (args != null && args.length >= 1) {
						...
						commandData.getMessageEvent()
							.respond("Usage: !challengefor <person>");
					}
				} else {
					commandData.getMessageEvent()
						.respond("I'm sorry. I don't do that here.");
	...
	private void issueChallenge(Channel channel, String target) {
		...
		channel.send()
			.action(String.format("challenges %s: %s", target, tagReplacer.doTagReplacment(Tim.db.dynamic_lists.get("challenge_templates")
				.get(Tim.rand.nextInt(
					Tim.db.dynamic_lists.get(
						"challenge_templates")
						.size())))));
	}
	...
	public void helpSection(MessageEvent event) {
		...
		String[] strs = {
			"Challenge Commands:", "    !challenge - Request a challenge", "    !challengefor <name> - Challenge someone else",
		};
