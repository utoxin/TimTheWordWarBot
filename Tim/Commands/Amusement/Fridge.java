...
public class Fridge implements ICommandHandler {
	...
	public boolean handleCommand(CommandData commandData) {
		if (handledCommands.contains(commandData.command)) {
			...
			if (cdata.commands_enabled.get("fridge")) {
				if (InteractionControls.interactWithUser(commandData.issuer, "fridge") && InteractionControls.interactWithUser(commandData.argString,
																															   "fridge")) {
					throwFridge(commandData.event.getChannel(), commandData.getUserEvent()
	...
	public void throwFridge(Channel channel, User sender, String[] args, Boolean righto) {
		try {
			...
			Thread.sleep(time);
			channel.send()
				   .action(sneakyMessage());

			time = Tim.rand.nextInt(3000) + 2000;
			Thread.sleep(time);
			channel.send()
				   .action(throwMessage(target, sender.getNick()));
	...
	private String sneakyMessage() {
		String[] messages = {
			"looks back and forth, then slinks off...", "slips into the kitchen to grab something...", "places an order with the local appliance store..."
		};
	...
	private String throwMessage(String target, String thrower) {
		String[] messages = {
			"hurls a%(acolor) fridge at %(target).", "hurls a%(acolor) fridge at %(thrower) and runs away giggling.",
			"trips and drops a%(acolor) fridge on himself.", "rigs a complicated mechanism, and drops a%(acolor) fridge onto %(target)",
