...
public class Defenestrate implements ICommandHandler {
	...
	public boolean handleCommand(CommandData commandData) {
		if (handledCommands.contains(commandData.command)) {
			...
			if (cdata.commands_enabled.get("defenestrate")) {
				if (((commandData.args == null || commandData.args.length == 0) && InteractionControls.interactWithUser(commandData.issuer, "defenestrate"))
					|| (commandData.args != null && commandData.args.length > 0 && InteractionControls.interactWithUser(commandData.args[0], "defenestrate"))) {
					defenestrate(commandData.event.getChannel(), commandData.getUserEvent()
																			.getUser(), commandData.args, true);
				} else {
	...
	public void defenestrate(Channel channel, User sender, String[] args, Boolean righto) {
		try {
			...
			String act;
			act = Tim.db.dynamic_lists.get("defenestration_starter")
									  .get(Tim.rand.nextInt(Tim.db.dynamic_lists.get("defenestration_starter")
																				.size()));
			channel.send()
				   .action(tagReplacer.doTagReplacment(act));

			int i = Tim.rand.nextInt(100);

			if (i > 33) {
				tagReplacer.setDynamicTag("target", target);
				act = Tim.db.dynamic_lists.get("defenestration_success")
										  .get(Tim.rand.nextInt(Tim.db.dynamic_lists.get("defenestration_success")
																					.size()));
			} else {
				tagReplacer.setDynamicTag("target", sender.getNick());
				act = Tim.db.dynamic_lists.get("defenestration_failure")
										  .get(Tim.rand.nextInt(Tim.db.dynamic_lists.get("defenestration_failure")
																					.size()));
			}
