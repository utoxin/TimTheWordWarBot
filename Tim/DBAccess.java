...
public class DBAccess {
	...
	public void removeFromChannelGroup(String group, ChannelInfo channel) {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			PreparedStatement s = con.prepareStatement(
				"DELETE `channel_groups`.* FROM `channel_groups` INNER JOIN `channels` ON (`channel_groups`.`channel_id` = `channels`.`id`) WHERE `channels`"
				+ ".`channel` = ? AND `channel_groups`.`name` = ?");
			s.setString(1, channel.channel);
			s.setString(2, group);
			s.executeUpdate();
			s.close();

			this.channel_groups.get(group.toLowerCase())
							   .remove(channel);
	...
	void joinChannel(Channel channel) {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			if (!this.channel_data.containsKey(channel.getName()
													  .toLowerCase())) {
				ChannelInfo new_channel = new ChannelInfo(channel.getName()
																 .toLowerCase());
				new_channel.setDefaultOptions();

				this.channel_data.put(channel.getName()
											 .toLowerCase(), new_channel);
			}

			this.saveChannelSettings(this.channel_data.get(channel.getName()
																  .toLowerCase()));
		} catch (SQLException ex) {
			Tim.printStackTrace(ex);
		}
		...
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			try (Statement s = con.createStatement()) {
				...
				while (rs.next()) {
					...
					ci.setReactiveChatter(rs.getFloat("reactive_chatter_level"), rs.getFloat("chatter_name_multiplier"));

					ci.setRandomChatter(rs.getFloat("random_chatter_level"));

					ci.setTwitterTimers(rs.getFloat("tweet_bucket_max"), rs.getFloat("tweet_bucket_charge_rate"));
		...
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			while (rs.next()) {
				if (rs.getString("type")
					...
					this.soft_ignore_list.add(rs.getString("name")
												.toLowerCase());
				}
			}
		} catch (SQLException ex) {
	...
	private void getChannelGroups() {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			while (rs.next()) {
				if (!this.channel_groups.containsKey(rs.getString("name")
													   .toLowerCase())) {
					this.channel_groups.put(rs.getString("name")
											  .toLowerCase(), new HashSet<>());
				}

				this.channel_groups.get(rs.getString("name")
										  .toLowerCase())
	...
	private void getDynamicLists() {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			while (rs.next()) {
				...
				this.dynamic_lists.get(rs.getString("name"))
	...
	public void create_war(WordWar war) {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			PreparedStatement s = con.prepareStatement(
				"INSERT INTO `new_wars` (`year`, `uuid`, `channel`, `starter`, `name`, `base_duration`, `base_break`, `total_chains`, `current_chain`, "
				+ "`start_epoch`, `end_epoch`, `randomness`, `war_state`, `created`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())",
	...
	public void updateWar(WordWar war) {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			PreparedStatement s = con.prepareStatement(
				"UPDATE `new_wars` SET `current_chain` = ?, `start_epoch` = ?, `end_epoch` = ?, `war_state` = ? WHERE `uuid` = ?");
		...
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			while (rs.next()) {
				if (channel_data.get(rs.getString("channel")) != null) {
					WordWar war = new WordWar(rs.getShort("year"), rs.getShort("war_id"), UUID.fromString(rs.getString("uuid")), rs.getString("channel"),
											  rs.getString("starter"), rs.getString("name"), rs.getInt("base_duration"), rs.getInt("base_break"),
											  rs.getByte("total_chains"), rs.getByte("current_chain"), rs.getLong("start_epoch"), rs.getLong("end_epoch"),
											  rs.getBoolean("randomness"), WordWar.State.valueOf(rs.getString("war_state")));
	...
	public WordWar loadWar(String compositeId) {
		...
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			...
			while (rs.next()) {
				returnWar = new WordWar(rs.getShort("year"), rs.getShort("war_id"), UUID.fromString(rs.getString("uuid")), rs.getString("channel"),
										rs.getString("starter"), rs.getString("name"), rs.getInt("base_duration"), rs.getInt("base_break"),
										rs.getByte("total_chains"), rs.getByte("current_chain"), rs.getLong("start_epoch"), rs.getLong("end_epoch"),
										rs.getBoolean("randomness"), WordWar.State.valueOf(rs.getString("war_state")));
	...
	public void saveInteractionSettings() {
		try (Connection con = Tim.db.pool.getConnection(timeout)) {
			con.createStatement()
			   .execute("TRUNCATE `user_interaction_settings`");
