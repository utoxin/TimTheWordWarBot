...
class UserCommandListener extends ListenerAdapter {
	private ICommandHandler[] commandHandlers = {
		new InteractionControls(), Tim.story, Tim.challenge, Tim.amusement, new Summon(), new Defenestrate(), new Fridge(), Tim.raptors, new War()
	};
		...
		if (event.getUser() != null && !Tim.db.ignore_list.contains(event.getUser()
			...
			if (commandData.type == CommandData.CommandType.TIMMY_USER) {
				switch (commandData.command) {
					...
					case "ignore":
						if (commandData.args.length > 0 && (commandData.args[0].equals("soft") || commandData.args[0].equals("hard"))) {
							if (commandData.args[0].equals("hard")) {
								Tim.db.ignore_list.add(event.getUser()
															.getNick()
															.toLowerCase());
								event.respond("Fine. I didn't like you either. See if I talk to you ever again...");
							} else {
								Tim.db.soft_ignore_list.add(event.getUser()
																 .getNick()
																 .toLowerCase());
								event.respond("Okay, I'll stop bothering you. Sorry!");
							}
							Tim.db.saveIgnore(event.getUser()
												   .getNick()
												   .toLowerCase(), commandData.args[0]);
						} else {
					...
					case "unignore":
						if (Tim.db.soft_ignore_list.remove(event.getUser()
																.getNick()
																.toLowerCase())) {
							event.respond("Okay! Thanks! I'll try not to be /TOO/ annoying...");
							Tim.db.deleteIgnore(event.getUser()
													 .getNick()
													 .toLowerCase());
						} else {
					...
					case "credits":
						event.respond(
							"I was created by MysteriousAges in 2008 using PHP, and ported to the Java PircBot library in 2009. Utoxin started helping during "
							+ "NaNoWriMo 2010. Sourcecode is available here: https://github.com/utoxin/TimTheWordWarBot, and my NaNoWriMo profile page is "
							+ "here: http://nanowrimo.org/en/participants/timmybot");
						break;
	...
	private void printCommandList(MessageEvent event) {
		...
		String[] strs = {
			"I am a robot trained by the WordWar Monks of Honolulu. You have never heard of them. It is because they are awesome.", "General Commands:",
