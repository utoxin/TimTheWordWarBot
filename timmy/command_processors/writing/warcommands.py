...
class WarCommands(BaseCommand):
    ...
    def process(self, event: Event, command_data: CommandData) -> None:
        if command_data.command == 'endwar':
            self.respond_to_user(event, "The syntax for war-related commands has changed. Try '!war cancel' in the "
                                        "future.")
            ...
            self.respond_to_user(event, "A long time ago, in a novel far, far away...")
            return
        elif command_data.command == 'startwar' or command_data.command == 'chainwar':
            self.respond_to_user(event, "The syntax for war-related commands has changed. Use '!war start'")
            return
        elif command_data.command == 'listall':
            self.respond_to_user(event, "The syntax for war-related commands has changed. Try '!war list all' in the "
                                        "future.")
            command_data.command = 'war'
            ...
            self.respond_to_user(event, "The syntax for war-related commands has changed. Try '!war list' in the "
                                        "future.")
            command_data.command = 'war'
            ...
            self.respond_to_user(event, "The syntax for war-related commands has changed. Try '!war join' in the "
                                        "future.")
            command_data.command = 'war'
            ...
            self.respond_to_user(event, "The syntax for war-related commands has changed. Try '!war leave' in the "
                                        "future.")
            command_data.command = 'war'
    ...
    def _start_handler(self, event: Event, command_data: CommandData) -> None:
        if command_data.arg_count >= 2:
            ...
            try:
                ...
                self.respond_to_user(event, "I couldn't understand the duration. Was it numeric?")
                return

            if duration < 60:
                self.respond_to_user(event, "Duration must be at least 1 minute.")
                return
                ...
                while i < command_data.arg_count:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            self.respond_to_user(event, "I didn't understand the start delay. Was it numeric?")
                    ...
                    if match:
                        ...
                        try:
                            ...
                            self.respond_to_user(event, "I didn't understand the chain count. Was it numeric?")
                            from timmy.utilities import irc_logger
                            irc_logger.log_message(
                    ...
                    if match:
                        ...
                        try:
                            ...
                            self.respond_to_user(event, "I didn't understand the break duration. Was it numeric?")
                            from timmy.utilities import irc_logger
                            irc_logger.log_message(
                    ...
                    if match:
                        ...
                        try:
                            ...
                            self.respond_to_user(event, "I didn't understand the randomness flag. Was it numeric?")
            ...
            if re.match("^\\d+$", war_name):
                self.respond_to_user(event, "War names must be more than a number. It's possible you meant to specify "
                                            "the start delay. Try: !war start {} delay:{}".format(command_data.args[1],
                                                                                                  war_name))
                return
            ...
            if to_start > 0:
                self.respond_to_user(event, "Your word war, {}, will start in {:.1f} minutes. The ID is "
                                            "{:d}-{:d}.".format(war.get_name(), to_start / 60, war.year, war.war_id))
        else:
            self.respond_to_user(event, "Usage: !war start <duration in minutes> [<options>] [<name>]")
            self.respond_to_user(event, "Options for all wars: delay:<minutes>")
            self.respond_to_user(event, "Options for chain wars: chains:<chain count>, random:1, break:<minutes>")
            self.respond_to_user(event, "Example: !war start 10 delay:5 Let's Write!")

    def _cancel_handler(self, event: Event, command_data: CommandData) -> None:
        if command_data.arg_count > 1:
            ...
            if war_by_name:
                self._remove_war(event, command_data, war_by_name)
            elif war_by_id:
                self._remove_war(event, command_data, war_by_id)
            else:
                self.respond_to_user(event, "I don't know of a war with the name or ID '{}'.".format(name))
        else:
            self.respond_to_user(event, "Syntax: !war cancel <name or id>")

    def _join_handler(self, event: Event, command_data: CommandData) -> None:
        war = self._find_join_leave_war(event, command_data)
        ...
        if war is not None:
            if war.state == WarState.PENDING or war.state == WarState.ACTIVE:
                ...
                self.respond_to_user(event, "You have joined the war.")
            else:
                self.respond_to_user(event, "That war is over or cancelled. Sorry!")

    def _report_handler(self, event: Event, command_data: CommandData) -> None:
        if command_data.arg_count < 2:
            self.respond_to_user(event, "Usage: !war report <wordcount> [<war id>]")
            self.respond_to_user(event, "Note: Wordcount should be words written during the war, not total count. The "
                                        "war id is optional. The last completed war will be selected by default.")
        else:
            ...
            if len(command_data.args) == 3:
                ...
                self.respond_to_user(event, "I don't know which war finished last. Try providing the War ID")
                return

            if user_data is None or not user_data.raptor_adopted:
                self.respond_to_user(event, "I'm sorry, you must be a registered user and have an adopted raptor to "
                                            "record your stats... (!raptor command)")
            elif war is None:
                self.respond_to_user(event, "That war couldn't be found...")
            elif war.state is WarState.CANCELLED:
                self.respond_to_user(event, "That war was cancelled. Sorry!")
            elif (war.total_chains == 1 and war.state is not WarState.FINISHED) or (war.total_chains > 1 and
                                                                                    war.current_chain == 1):
                self.respond_to_user(event, "Can't record an entry for a war that isn't complete. Sorry!")
            else:
                try:
                    wordcount = int(float(command_data.args[1]))
                except TypeError:
                    self.respond_to_user(event, "I didn't understand the wordcount. Was it numeric?")
                    from timmy.utilities import irc_logger
                    irc_logger.log_message(
                ...
                self.respond_to_user(event, f"{user_data.raptor_name} pulls out their {user_data.raptor_favorite_color}"
                                            f" notebook and makes a note of that wordcount.")

                if not channel_data.is_muzzled():
                    core.raptor_ticker.handle_war_report(user_data, war, wordcount)

    def _leave_handler(self, event: Event, command_data: CommandData) -> None:
        war = self._find_join_leave_war(event, command_data)
        ...
        if war is not None:
            ...
            self.respond_to_user(event, "You have left the war.")
        ...
        if len(core.war_ticker.active_wars) > 0:
            ...
            for war in core.war_ticker.active_wars:
                if all_wars or war.channel.lower() == command_data.channel.lower():
                    ...
                    self.respond_to_user(event, output)
                    responded = True

        if not responded:
            self.respond_to_user(event, "No wars are currently available.")

    def _find_join_leave_war(
            self, event: Event, command_data: CommandData
    ) -> Optional[WordWar]:
        if event.type == "privmsg":
            self.respond_to_user(event, "Sorry. This currently only works in regular channels.")
            return None
        else:
            channel_data = core.bot_instance.channels[command_data.channel]
        ...
        if command_data.arg_count == 1:
            if channel_data.newest_war_id != "":
                ...
                self.respond_to_user(event, "Hmmm. I don't seem to have a record of an upcoming war. Try providing a "
                                            "war ID?")
                self.respond_to_user(event, "Usage: !war {} [<war id>]".format(command_data.args[0]))
                return None
        else:
            war = self._find_war_by_id(command_data.args[1])

        if war is None:
            self.respond_to_user(event, "That war id was not found.")
    ...
    def _remove_war(self, event: Event, command_data: CommandData, war: WordWar) -> None:
        if command_data.issuer.lower() == war.starter.lower():
            core.war_ticker.active_wars.remove(war)
            war.cancel_war()
            self.respond_to_user(event, "{} has been ended.".format(war.get_name()))
        else:
            self.respond_to_user(event, "Only the starter of a war can end it early.")
