...
class DefenestrateCommand(BaseCommand):
    ...
    def defenestrate_command(self, connection: ServerConnection, event: Event, command_data: CommandData,
                             target: str) -> None:
        ...
        if random_choice > 33:
            second_message = text_generator.get_string("[defenestration_success]", {
                'target': target
            })
        else:
            second_message = text_generator.get_string("[defenestration_failure]", {
